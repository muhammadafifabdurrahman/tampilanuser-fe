...
export default function KasirOrdersItems() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [ordersItemsData, menusData, ordersData] = await Promise.all([getOrdersItems(), getMenus(), getOrders()]);
      setOrdersItems(ordersItemsData);
      setMenus(menusData);
                      ...
                      <td className="relative flex items-center justify-center px-4 py-3">
                        ...
                        {openDropdownId === order_item.id && (
                          <div id="apple-imac-27-dropdown" className="absolute right-0 z-10 mt-2 bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-700 dark:divide-gray-600" style={{ top: "100%", right: "0" }}>
                            <ul className="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby={`dropdown-button-${order_item.id}`}>
                              <li>
                                <Link to={`/admin/order_items/edit/${order_item.id}`} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                  Edit
                                </Link>
                              </li>
                            </ul>
