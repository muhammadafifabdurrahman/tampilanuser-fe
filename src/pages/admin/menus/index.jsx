...
export default function AdminMenus() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [menusData, categoriesData] = await Promise.all([getMenus(), getCategories()]);
      setMenus(menusData);
                      ...
                      <td className="relative flex items-center justify-center px-4 py-3">
                        ...
                        {openDropdownId === menu.id && (
                          <div id="apple-imac-27-dropdown" className="absolute right-0 z-10 mt-2 bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-700 dark:divide-gray-600" style={{ top: "100%", right: "0" }}>
                            <ul className="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby={`dropdown-button-${menu.id}`}>
                              <li>
                                <Link to={`/admin/books/edit/${menu.id}`} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                  Edit
                                </Link>
                              </li>
                            </ul>
